...
def run():
    with app.app_context():
        ...
        # ----------------------------------------------------------------- #
        # Approvals (guardrail outcomes)
        # ----------------------------------------------------------------- #
        # r4 -> NEEDS_APPROVAL (above auto-approve limit)
        a4 = ApprovalRequest(
            request_id=r4.id, amount=72000, payee="DataVault Enterprise",
            status="BLOCKED", policy_decision="BLOCK", policy_rule="hard_spend_cap",
            agent_reason="Amount $72,000 exceeds the hard spend cap of $50,000. Escalation required.",
        )
        r4.status = "sent_to_guardrail"

        # r2 -> NEEDS_APPROVAL
        a2 = ApprovalRequest(
            request_id=r2.id, amount=58000, payee="Meridian Assurance",
            status="BLOCKED", policy_decision="BLOCK", policy_rule="hard_spend_cap",
            agent_reason="Amount $58,000 exceeds the hard spend cap of $50,000. Escalation required.",
        )
        r2.status = "sent_to_guardrail"

        # r1 -> NEEDS_APPROVAL (38.5k, above 5k auto limit, below 50k cap)
        a1 = ApprovalRequest(
            request_id=r1.id, amount=38500, payee="NimbusCloud",
            status="NEEDS_APPROVAL", policy_decision="NEEDS_APPROVAL",
            policy_rule="above_auto_approve_limit",
            agent_reason="Amount $38,500 is above the auto-approve limit of $5,000; human sign-off required.",
        )
        r1.status = "sent_to_guardrail"

        db.session.add_all([a1, a2, a4])
        db.session.commit()
        ...
        # ----------------------------------------------------------------- #
        report = AuditReport(
            title="June 2026 Audit Report",
            total_spend=29700,
            total_savings=14800,
            reconciliation_status="discrepancy",
            compliance_replay_result="pass",
            notes="One amount mismatch and one missing-approval exception flagged for review.",
        )
        report.exceptions = [
            AuditException(exception_type="amount_mismatch", amount=300,
                           description="Stripe charged $1,500 vs ledger $1,200 for CloudFlare renewal.",
                           transaction_id="txn_cf_001"),
            AuditException(exception_type="missing_approval", amount=8600,
                           description="AWS payment posted without a recorded approval signature.",
                           transaction_id="txn_aws_330"),
            AuditException(exception_type="stripe_only_charge", amount=450,
                           description="Stripe charge with no matching ledger entry.",
                           transaction_id="txn_str_555"),
        ]
